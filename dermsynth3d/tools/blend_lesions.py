...
class BlendLesions:
    ...
    def blend_lesions(self):
        ...
        pbar1 = tqdm(
            total=self.deepblend3d.blended3d.lesion_params().shape[0],
            leave=True,
            desc="Blending Lesions",
        )
        for _, params in self.deepblend3d.blended3d.lesion_params().iterrows():
            ...
            pbar = tqdm(
                total=self.num_iter,
                desc=f"Optimizing at face_idx ({int(self.deepblend3d.params.face_idx)})",
                leave=False,
                nrows=40,
            )
        ...
        merged_texture_np = (
            self.deepblend3d.postprocess_blended_texture_image()
        )

        # Save the final blended textures to disk.
        blended_filename = (
            Path(self.SAVE_DIR)
            / f"model_highres_0_normalized_blended_{self.ext}.png"
        )
        self.blend3d.save_blended_texture_image(
            merged_texture_np, print_filename=False, filename=blended_filename
        )
