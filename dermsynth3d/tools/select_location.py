...
class SelectAndPaste:
    ...
    def __init__(
        ...
        self.config = config
        self.SAVE_DIR = (
            Path(config["generate"]["save_dir"].split("/")[0])
            / "processed_textures"
        )
        ...
        self.original_texture_image_tensor = self.blended3d.texture_image(
            astensor=True
        ).to(self.device)
    ...
    def load_mesh(self):
        ...
        self.mesh = load_objs_as_meshes(
            [self.mesh_filename], device=self.device
        )
    ...
    def paste_on_locations(self):
        ...
        pbar = tqdm(
            total=self.num_paste,
            desc="Pasting lesions",
            ncols=40,
            dynamic_ncols=True,
            leave=True,
        )
        ...
        while lesion_mask_id <= self.num_paste:
            ...
            if max_depth_diff > 0.02:
                ...
                pbar.set_description(
                    f"Skipping {face_idx} due to high depth change.",
                    refresh=True,
                )
                continue

            accepted = self.paster.paste_masks_and_texture_images(
                lesion_mask_id
            )
            if not accepted:
                # print("Skipping {} as overlaps with existing lesion.".format(face_idx))
                pbar.set_description(
                    f"Skipping {face_idx} as overlaps with existing lesion.",
                    refresh=True,
                )
            ...
            self.selected_params.append(
                {
                    "face_idx": face_idx,
                    "normal_weight": self.paster.mesh_renderer.params[
                        "normal_weight"
                    ],
            ...
            pbar.set_description(
                f"Accepted {lesion_mask_id} lesion", refresh=True
            )
            pbar.update(1)
            lesion_mask_id += 1
    ...
    def save_textures(self):
        ...
        pasted_filename = (
            self.SAVE_DIR / f"model_highres_0_normalized_pasted_{self.ext}.png"
        )
        dilated_filename = (
            self.SAVE_DIR
            / f"model_highres_0_normalized_dilated_{self.ext}.png"
        )
        lesion_mask_filename = self.SAVE_DIR / f"lesion_mask_{self.ext}.png"
        lesion_dilated_mask_filename = (
            self.SAVE_DIR / f"lesion_dilated_mask_{self.ext}.png"
        )
        params_filename = self.SAVE_DIR / f"lesion_params_{self.ext}.csv"

        self.blended3d.save_pasted_texture_image(
            self.paster.pasted_texture,
        ...
        self.blended3d.save_dilated_texture_image(
            ...
            print_filepath=False,
            filename=dilated_filename,
        )

        # Save the lesion mask for the texture image.
        self.blended3d.save_lesion_texture_mask(
            self.paster.lesion_mask.squeeze(),
            print_filename=False,
            filename=lesion_mask_filename,
        )
        self.blended3d.save_dilated_texture_mask(
            self.paster.lesion_dilated_mask.squeeze(),
            print_filepath=False,
            filename=lesion_dilated_mask_filename,
        )
        ...
        self.blended3d.save_lesion_params(
            final_params_df, print_filepath=True, filename=params_filename
        )
